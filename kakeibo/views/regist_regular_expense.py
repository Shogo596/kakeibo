...
def regist_regular_expense(request):
    ...
    detail_records = 支出明細.objects\
        .filter(削除フラグ='0', 対象年月日__startswith=yyyymm, 支出分類コード__固定変動区分='0').order_by('id').reverse()
    ...
    for classify_row in classify_records:
        for person_row in person_records:
            ...
            detail_row = detail_records\
                .filter(支出分類コード=regular_form_data.classify, 対象者コード=regular_form_data.person).first()
            ...
            if detail_row is not None:
                ...
                regular_rows = regular_records\
                    .filter(支出分類コード=regular_form_data.classify, 対象者コード=regular_form_data.person)
